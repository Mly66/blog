...
- **事务隔离级别 **（从低到高）
1. 读未提交（Read Uncommitted）（可能出现脏读）
2. 读已提交（Read Committed）（SQL Server 默认，可能出现不可重复读）
3. 可重复读（Repeatable Read）（MySQL 默认，可能出现幻读）
4. 串行化（Serializable）（最严格，但性能最差）
...
- **ReadView 版本判断规则**
1. 事务 ID **小于最小活跃事务 ID** （可见）
2. 事务 ID **大于当前事务 ID** （不可见）
3. 事务 ID **在活跃事务列表中** （不可见）

...
- **支持动态 SQL**：可用 `if`、`choose`、`where` 等标签动态拼接 SQL。
- **缓存机制**：提供 **一级缓存 & 二级缓存**，提高查询效率。
...
### 5. MyBatis 工作流程
1. **读取 MyBatis 配置文件**（`mybatis-config.xml`），创建 `SqlSessionFactory`。
2. **通过 `SqlSessionFactory` 获取 `SqlSession`**，建立数据库连接。
3. **调用 Mapper 执行 SQL**，查询数据。
4. **返回结果，MyBatis 解析 SQL 并封装为对象**。
...
### 总结
1. MyBatis 是 **SQL 映射框架**，支持**手写 SQL & 高性能查询**。
2. **一级缓存 & 二级缓存** 提高查询效率，但使用需谨慎。
3. **动态 SQL** 通过 `<if>`、`<foreach>` 等标签提高灵活性。
4. **插件机制** 可用于**拦截 SQL、分页、日志等功能**。
